import { apiClient, handleApiError } from "./client";

export interface QuotationLineItem {
  materialId?: string;
  description: string;
  quantity: number;
  unitPrice: number;
  total: number;
}

export interface Quotation {
  id: string;
  quotationNumber: string;
  clientName: string;
  clientAddress?: string;
  clientEmail?: string;
  clientPhone?: string;
  projectTitle?: string;
  projectReference?: string;
  currency: string;
  preparedBy: string;
  lineItems: QuotationLineItem[] | string;
  subtotal: string;
  nhilRate?: string;
  getfundRate?: string;
  paymentTerms?: string;
  deliveryTerms?: string;
  vatRate: string;
  vatAmount: string;
  total: string;
  deliveryPeriod?: string;
  validityPeriod?: number;
  termsAndConditions?: string;
  status: "draft" | "sent" | "accepted" | "rejected" | "expired";
  createdAt: string;
  updatedAt: string;
}

export const quotationsApi = {
  getAll: async (params?: {
    search?: string;
    status?: string;
    limit?: number;
    offset?: number;
  }): Promise<Quotation[]> => {
    try {
      const response = await apiClient.get<{ quotations: Quotation[] }>(
        "/quotations",
        { params }
      );
      return response.data.quotations;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  getById: async (id: string): Promise<Quotation> => {
    try {
      const response = await apiClient.get<{ quotation: Quotation }>(
        `/quotations/${id}`
      );
      return response.data.quotation;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  create: async (data: Partial<Quotation>): Promise<Quotation> => {
    try {
      const response = await apiClient.post<{ quotation: Quotation }>(
        "/quotations",
        data
      );
      return response.data.quotation;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  update: async (id: string, data: Partial<Quotation>): Promise<Quotation> => {
    try {
      const response = await apiClient.put<{ quotation: Quotation }>(
        `/quotations/${id}`,
        data
      );
      return response.data.quotation;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  delete: async (id: string): Promise<void> => {
    try {
      await apiClient.delete(`/quotations/${id}`);
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  exportPDF: async (id: string, view: boolean = false): Promise<string> => {
    try {
      const response = await apiClient.get(`/quotations/${id}/pdf?view=${view}`, {
        responseType: "blob",
      });
      const blob = new Blob([response.data], { type: "application/pdf" });
      const url = window.URL.createObjectURL(blob);
      
      if (!view) {
        // Download mode
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", `quotation-${id}.pdf`);
        document.body.appendChild(link);
        link.click();
        link.remove();
        window.URL.revokeObjectURL(url);
        return "";
      }
      
      // View mode - return URL for viewing
      return url;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },

  sendEmail: async (id: string, data: { recipientEmail: string; subject?: string; body?: string }): Promise<void> => {
    try {
      await apiClient.post(`/quotations/${id}/email`, data);
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  },
};
